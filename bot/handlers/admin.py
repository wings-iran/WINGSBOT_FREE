...
async def admin_xui_choose_inbound(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build message body; for X-UI/3x-UI/TX-UI we won't include sub link
    ...
    try:
        sent = False
        if (panel_row.get('panel_type') or '').lower() in ('3xui','3x-ui','3x ui','xui','x-ui','sanaei','alireza','txui','tx-ui','tx ui') and hasattr(api, 'get_configs_for_user_on_inbound'):
            try:
                ...
                sent = False
        if not sent:
            # If X-UI/3x-UI/TX-UI and configs failed, do NOT send sub link; send message only
            if (panel_row.get('panel_type') or '').lower() in ('3xui','3x-ui','3x ui','xui','x-ui','sanaei','alireza','txui','tx-ui','tx ui'):
